...
function inicializarGlobalDropzone() {
  ...
  window.addEventListener("drop", async (e) => {
    ...
    if (jsonFiles.length > 0) {
      jsonFiles.forEach(jf => processarArquivoJson(jf));
    }

    if (videos.length > 0) {
      videos.sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true, sensitivity: 'base' }));
      for (let i = 0; i < Math.min(videos.length, CANAIS.length); i++) {
        const canalId = CANAIS[i].id;
        await processarArquivo(canalId, videos[i]);
      }
      setMsg(`${videos.length} vídeo(s) distribuído(s) automaticamente para os canais!`);
    }
  });
}
