...
function atualizarAgendamento(canalId){const dataEl=document.getElementById(`data-${canalId}`);const horaEl=document.getElementById(`hora-${canalId}`);if(!dataEl||!horaEl||!estadoCanais[canalId])return;const data=dataEl.value;const hora=horaEl.value;estadoCanais[canalId].agendamento = data && hora ? `${data}T${hora}:00-03:00` : "";}
